...
async function createNewWallet() {
    ...
    } catch (error) { console.error("Error:", error); showToast('Failed to create wallet', 'error'); }
...
async function claimMilestone(referrals) {
    ...
    userData.balances.USDT += reward; userData.totalUsdtEarned += reward;
    milestone.claimed = true; saveUserData();
    renderAirdrop(); renderWallet(); updateTotalBalance();
...
function showHistory() {
    ...
    if (txs.length === 0) list.innerHTML = '<div style="text-align:center;padding:40px;">📭 No transactions</div>';
...
function showNotifications() {
    ...
    if (notifications.length === 0) list.innerHTML = '<div style="text-align:center;padding:40px;">📭 No notifications</div>';
...
function copyDepositAddress() { const address = document.getElementById('depositAddress')?.innerText; if (address) copyToClipboard(address); }
function copyAddress() { const address = document.getElementById('receiveAddress')?.innerText; if (address) copyToClipboard(address); }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await loadUserData();
    setTimeout(() => { const splash = document.getElementById('splashScreen'); if (splash) splash.classList.add('hidden'); }, 1500);
...
window.showReceiveModal = showReceiveModal; window.showSwapModal = showSwapModal;
window.showHistory = showHistory; window.showNotifications = showNotifications;
window.showAdminPanel = showAdminPanel; window.closeModal = closeModal;
window.toggleLanguage = toggleLanguage; window.toggleTheme = toggleTheme;
window.logout = logout; window.copyInviteLink = copyInviteLink;
window.claimMilestone = claimMilestone; window.sendTransaction = sendTransaction;
window.submitWithdraw = submitWithdraw; window.createNewWallet = createNewWallet;
window.importWallet = importWallet; window.showImportModal = showImportModal;
